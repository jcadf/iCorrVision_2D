########################################################################################################################
# iCorrVision-2D Grabber Module GUI                                                                                    #
...
'''
    ...
    The iCorrVision-2D Grabber Module GUI is free software: you can redistribute it and/or modify
    it under the terms of the GNU General Public License as published by
    the Free Software Foundation, either version 3 of the License, or
    (at your option) any later version.

    The iCorrVision-2D Grabber Module GUI is distributed in the hope that it will be useful,
...
if __name__ == '__main__':
    ...
    # GUI title:
    menu.title('iCorrVision-2D Grabber Module ' + V)
    menu.iconbitmap(CurrentDir + '\static\ImgAcquisition2D.ico')
    ...
    global amplifyImg; amplifyImg = BooleanVar(menu)
    ...
    # iCorrVision-2D Grabber logo:
    image_logo = Image.open(os.path.join(CurrentDir, 'static', 'iCorrVision-2D Grabber.png'))
    ...
    select_btn = tk.Button(menu, text='Select folder', width=19, height=1, bg='#DADDE3', activebackground='#ccd9e1',
                         fg='#282C34', command=lambda: module.folder(console, folder_status, selectionFolder, k_snapshot))
    select_btn.place(x=xinit, y=yinit+dyinit)
    folder_status = Entry(menu, bg='red')
    folder_status.place(x=xinit + 142, y=yinit +dyinit- 1, width=8, height=27)

    # Width in pixels:
    Label(menu, text='Width (pixels):', bg='#99b3c3', fg='#282C34', font=('Heveltica', 10)).place(x=xinit, y=yinit + dyinit*2)
    Entry(menu, textvariable=Width, bg='#ccd9e1', font=('Heveltica', 10)).place(x=xinit, y=yinit + 23 + dyinit*2, width=150,
                                                                                height=25)
    ...
    Label(menu, text='Frames per second (FPS):', bg='#99b3c3', fg='#282C34', font=('Heveltica', 10)).place(x=xinit,
                                                                                                           y=yinit + dyinit * 5)
    Entry(menu, textvariable=Fps, bg='#ccd9e1', font=('Heveltica', 10)).place(x=xinit, y=yinit + 23 + dyinit * 5, width=150,
                                                                              height=25)
    ...
    format_image.place(x=xinit, y=yinit + 23 + dyinit * 8, width=150, height=25)
    format_image.current(1)

    # Preview cameras:
    tk.Button(menu, text='Preview', width=19, height=1, bg='#DADDE3', activebackground='#ccd9e1', fg='#282C34',
              command=lambda: module.preview(menu, preview_test, visualize_status, console, Width, Height, Exposure, Fps, amplifyImg,
            canvas_img, gridvar1, gridvar2)).place(x=xinit, y=yinit + dyinit * 12 + 23 * 2)
    ...
    Entry(menu, textvariable=TimeInit, bg='#ccd9e1', font=('Heveltica', 10)).place(x=xinit, y=yinit + 23 * 2 + dyinit * 10,
                                                                                   width=150, height=25)
    ...
    tk.Button(menu, text='Snapshot', width=20, height=1, bg='#DADDE3', activebackground='#ccd9e1', fg='#282C34',
              command=lambda: module.snapshot(console, selectionFolder, preview_test, k_snapshot, TypeImg)).place(x=xinit + 661, y=yinit + dyinit * 9 + 23 * 2)
    ...
    tk.Button(menu, text='Start capture', width=20, height=1, bg='#DADDE3', activebackground='#ccd9e1', fg='#282C34',
              command=lambda: module.start_capture(menu, console, visualize_status, preview_test, Width, Height, Exposure, Fps, selectionFolder, TypeImg,
                  canvas_img, nImagesInit, TimeInit, TimeAcquisition)).place(x=xinit + 661, y=yinit + dyinit * 10 + 23 * 2)
    ...
    tk.Button(menu, text='Stop capture', width=20, height=1, bg='#DADDE3', activebackground='#ccd9e1', fg='#282C34',
              command=lambda: module.stop_capture(menu, console, selectionFolder, folder_status)).place(x=xinit + 661, y=yinit + dyinit * 11 + 23 * 2)
    tk.Button(menu, text='Close', width=20, height=1, bg='#DADDE3', activebackground='#ccd9e1', fg='#282C34',
              command=lambda: module.close(menu)).place(x=xinit + 661, y=yinit + dyinit * 12 + 23 * 2)
    ...
    # Canvas:
    canvas_img = tk.Label(menu, width=640, height=480, bg='#99b3c3', borderwidth=0, highlightthickness=0,
                           image=image_black_re)
    canvas_img.place(x=xinit + 170, y=yinit)

    # Open user guide pdf:
    tk.Button(menu, text='User guide', width=20, height=1, bg='#DADDE3', activebackground='#ccd9e1',
              fg='#282C34', command=lambda: module.openguide(CurrentDir)).place(x=xinit + 652, y=yinit +10)

    # Console:
    console = st.ScrolledText(menu, bg='#ccd9e1')
    console.place(x=xinit + 170, y=yinit + 23 * 2 + dyinit * 9, width=470, height=176)

    # Button to amplify the image:
    tk.Button(menu, text='Amplify', width=10, height=1, bg='#DADDE3', activebackground='#ccd9e1', fg='#282C34',
              command=lambda: Thread(target=module.amplifyI,args=(preview_test, amplifyImg)).start()).place(x=xinit + 722, y=yinit + dyinit * 8 + 23 * 2)
    ...
    Label(menu,
          text='Developer: João C. A. D. Filho, M.Sc. (joaocadf@id.uff.br) / Supervisors: Luiz C. S. Nunes, D.Sc. (luizcsn@id.uff.br) and José M. C. Xavier, P.hD. (jmc.xavier@fct.unl.pt)',
